...
export default async function EmailsPage() {
  ...
  return <main className="page">
    ...
    <section className="panel"><div className="table-wrap"><table><thead><tr><th>Última actividad</th><th>Organización / usuario</th><th>Proveedor</th><th>Asunto</th><th>Enviados</th><th>Recibidos</th><th>Pendientes</th><th>Estado</th></tr></thead><tbody>
      {page.items.map(row => <tr key={row.id}><td>{new Date(row.last_message_at || row.created_at).toLocaleString("es-CL")}</td><td><strong>{row.organization}</strong><small>{row.user_name} · {row.user_email}</small></td><td><strong>{row.proveedor_nombre || "Sin proveedor"}</strong><small>{row.proveedor_email || "Sin email"}</small></td><td>{row.subject || "Sin asunto"}</td><td>{row.messages.outbound}</td><td>{row.messages.inbound}</td><td>{row.messages.pending}</td><td><StatusPill tone={row.estado === "complete" || row.estado === "closed" ? "green" : row.estado === "failed" ? "red" : "amber"}>{row.estado}</StatusPill></td></tr>)}
      {!page.items.length && <tr><td colSpan={8}>Todavía no hay conversaciones registradas.</td></tr>}
