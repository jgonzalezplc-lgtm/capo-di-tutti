...
const cell = (resource: string | undefined, row: Record<string, unknown>, column: string) => {
  if (resource === "projects" && column === "id") return <Link className="table-link" href={`/proyectos/${String(row.id)}`}>Abrir proyecto</Link>;
  if (resource === "quotes" && column === "project_name" && row.project_id) return <Link className="table-link" href={`/proyectos/${String(row.project_id)}`}>{display(row.project_name)}</Link>;
  if (resource === "email_conversations" && column === "id") return <Link className="table-link" href={`/correos/${String(row.id)}`}>Ver historial</Link>;
